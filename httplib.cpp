...
Error wait_until_socket_is_ready(socket_t sock, time_t sec,
  ...
  auto fdsw = fdsr;
  auto fdse = fdsr;e_t read_timeout_sec, time_t read_timeout_usec,
...
bool brotli_compressor::compress(const char *data, size_t data_length,
  ...
  for (;;) {
    ...
    auto next_out = buff.data();I_DECODER_RESULT_ERROR) {
    return 0;
  }
...
std::pair<std::string, std::string> make_digest_authentication_header(
  ...
  std::string response;
  {
    ...
    if (qop.empty()) {
      ...
      response = H(H(A1) + ":" + auth.at("       return true;
      }
    }
  }
  return false;
...
bool Server::set_mount_point(const std::string &mount_point,
                                    consterror_handler(Handler handler) {
...
bool Server::handle_file_request(const Request &req, Response &res,
                                        bool head) {
  for (const auto                            int socket_flags,
  ...
  try {
    ...
    if (exception_handler_) {
      ...
      for (size_t i = 0; s[i]; i++) {
        ...
        case '\r': val += "\\r"; break;
        case '\n': val += "\\n"; break;
        default: val += s[i]; breakocess_request(strm, close_connection, connection_closed,
                               nullptr);
      });
...
bool ClientImpl::send_(Request &req, Response &res, Error &error) {
  ...
  auto se = detail::scope_exit([&]() {
    ...
    if (socket_should_be_closed_when_request      return detail::redirect(cli, req, res, path, location, error);
#else
      return false;
#endif
    } else {
  ...
  if (compress_ && !content_provider_without_length) {
    ...
    if (content_provider) {
      ...
      if (!compressor.compress(body, content_length, true,
                               [&](c, const std::string &path, const Headers &headers,
